      ...
      <main className="flex-1">
        <div className="mx-auto flex max-w-6xl flex-col gap-8 px-6 py-16">
          <header className="space-y-2">
            <h1 className="text-4xl font-bold tracking-tight">Triage contracts</h1>
            <p className="text-muted-foreground">
              Upload an NDA, get a typed clause list back. No agents, no
              playbook, no redline — just the mechanical ingest → parse
              → classify pipeline (Phase 1).
            </p>
          </header>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                The classifier runs without a real LLM key in Phase 1
                (it falls back to a deterministic rule-based pass), so
                results are reproducible. Confidence reflects the
                classifier's certainty, not the quality of the
                classification.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <Dropzone onFile={handleFile} disabled={ingest.isPending} />
              {selectedFile && (
                <p className="text-xs text-muted-foreground">
